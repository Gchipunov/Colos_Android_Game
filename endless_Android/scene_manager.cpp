...
#include "common.hpp"
#include "scene.hpp"
#include "colo_engine.h" // extern ColoEngine *g_ColoEngine ;
...
void SceneManager::DoFrame() {
  ...
  if (mHasGraphics && mCurScene) {
    mCurScene->DoFrame();
    //  LOGD("mgr-> Yes mCurScene->DoFrame() ;mHasGraphics:%d,mCurScene: %x ", mHasGraphics, mCurScene);

  }
  else
  {
      LOGD("mgr-> No mCurScene->DoFrame() ;mHasGraphics:%d,mCurScene: %x ", mHasGraphics, mCurScene);
...
// m_assetManager = app_->activity->assetManager;
AAssetManager *SceneManager::GetAssetManager() { 
if(g_ColoEngine)
{
return	g_ColoEngine->GetAndroidApp()->activity->assetManager;
}
...
void SceneManager::OnPointerMove(int pointerId, float x, float y, float dx, float dy)
{
if (mHasGraphics && mCurScene) {
    mCurScene->OnPointerMove( pointerId,  x,  y,  dx,  dy);
  }
